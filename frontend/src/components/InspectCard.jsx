...
export default function InspectCard({ card, user }) {
  ...
  const handleApproveCard = async () => {
    try {
      await http.post(`${APIURL}/admin/accept/${card.id}`);
  ...
  const handleRejectCard = async () => {
    try {
      await http.post(`${APIURL}/admin/reject/${card.id}`);
      ...
      <div className="card-container">
        <Container className="d-flex flex-column gap-2">
          ...
          <Row>
            <img
                style={{ height: "200px" }}
                src={card?.imagePath || "path/to/default/image.jpg"} // Add a default image path
                alt="Card"
            />
          </Row>
          <Row>
